...
<body>
  ...
  <div class="container mt-5">
    <div class="row mt-5">
      <div class="col-4"></div>
      <div class="col-4">
        <form class='card Regular shadow p-4' method='POST' action="{{route('createSubject')}}">
          @csrf
          <div class="form-group mb-1">
            <label for="exampleInputPassword1">Jańa taraw qosıw:</label>
            <input type="text" name='name' class="form-control mt-2" id="exampleInputPassword1" placeholder="jan'a taraw kiritiń">
              @error('name')
                <span class="text-danger p-1">{{$message}}</span>
              @enderror
          </div>
          <input type='submit' class='btn btn-outline-primary mt-3' value='Add Subject'>
        </form>
      </div>
      <div class="col-4"></div>
    </div>
